    ...
    for day in range(DAYS_OF_OPERATION):
        ...
        if is_solar:
            ...
            if energy_in_batteries >= energy_from_batteries_to_pump:
                volume_pumped_m3 = water_to_pump  # Assume we can pump the entire catchment tank if we have enough energy.
                ...
                if energy_from_batteries_to_pump > 0:
                    ...
                    volume_pumped_m3 = water_to_pump * max(0.0, min(1.0, fraction_transferable))
...
def calculate_flow_rate_to_house(solution: Design):
    ...
    discriminant = X**2 - 4*W*Y
    # No need to check discriminant; W and X will always be positive,
    # and Y is guaranteed negative at this point (supply height > 0).
    # By the properties of quadratics, only the plus-root will be positive.
    Q = (-X + discriminant**0.5) / (2*W)
